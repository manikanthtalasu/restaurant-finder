...
class Restaurant extends DB
{
	...
	{
		$sql = "SELECT * FROM `restaurant` WHERE (`name` LIKE '%$data%' OR `address1` LIKE '%$data%' OR `address2` LIKE '%$data%' OR `city` LIKE '%$data%' OR  `state` LIKE '%$data%' OR `zipcode` LIKE '%$data%' OR `phone` LIKE '%$data%') AND `deleted_at` IS NULL AND `status` = 1;";
